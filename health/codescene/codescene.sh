    ...
    if needs_check "$file"; then
        ...
        case "$status" in
            ...
            "warning")
                warning_count=$((warning_count + 1))
                warning_files+=("$file")
                echo -e "${YELLOW}⚠️  $file ${BLUE}(cached)${NC}"
                if [ -n "$details" ]; then
                    echo -e "$details"
                fi
                echo ""
            ...
            "error")
                error_count=$((error_count + 1))
                error_files+=("$file")
                echo -e "${RED}❌ $file ${BLUE}(cached)${NC}"
                if [ -n "$details" ]; then
                    echo -e "$details"
                fi
                echo ""
...
if [ $error_count -gt 0 ] || [ $warning_count -gt 0 ]; then
    echo ""
    echo -e "${YELLOW}💡 Run 'cs check <file>' for detailed analysis${NC}"
    echo -e "${YELLOW}💡 Run './scripts/check-code-health.sh --full' to force full check${NC}"
    exit 1
fi
