...
export function validateGraph(graph: AnalysisGraph): ValidationResult {
  ...
  logger.debug(
    { caller: 'validateGraph' },
    'Starting validation',
    { nodeCount: graph.nodes.length, branchCount: graph.branches.length }
  );
  ...
  logger.debug(
    { caller: 'validateGraph' },
    'Validation complete',
    { isValid, isSolvable, errorCount: errors.length, warningCount: warnings.length }
  );
...
function checkSourcePresence(graph: AnalysisGraph): string | null {
  const hasSource = graph.branches.some(
    branch => branch.type === 'voltageSource' || branch.type === 'currentSource'
  );
...
function traceFundamentalLoop(
  ...
  return loopBranchIds
    .map(id => graph.branches.find(b => b.id === id))
...
function getSelectedTree(
  graph: AnalysisGraph
): { twigBranchIds: string[]; linkBranchIds: string[] } | undefined {
  return graph.allSpanningTrees.find(
...
function genericBFS(
  startNodeId: string,
  getNeighbors: (nodeId: string) => Array<{ nodeId: string }>
): Set<string> {
