 ...
 * @returns Branch IDs if found, null otherwise
 */
function findBranchesByMode(
  elementId: string,
  visualizationData: GraphVisualizationData
): string[] | null {
  if (visualizationData.mode === 'loops') {
    return tryFindBranches(
      elementId,
      id => findLoopBranches(id, visualizationData.loopDefinitions),
      'loop'
    );
  }
  ...
  if (visualizationData.mode === 'cutsets') {
    return tryFindBranches(
      elementId,
      id => findCutSetBranches(id, visualizationData.cutSetDefinitions),
      'cut-set'
    );
  }
 ...
 */
export function handleDefinitionClick(
  elementId: string,
  visualizationData: GraphVisualizationData
): string[] {
