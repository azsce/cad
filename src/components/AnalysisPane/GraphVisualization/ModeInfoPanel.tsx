...
function renderTreeMode(
  ...
  return (
    <Box>
      ...
      <Typography variant="body2" color="text.secondary">
        Links (Co-tree): {linkCount} = B-N+1 ={' '}
        {analysisGraph.branches.length.toString()}-{nodeCount.toString()}+1
      </Typography>
      <Divider sx={{ my: 1 }} />
      <Typography variant="body2" color="text.secondary">
...
function handleLoopsMode(
  ...
  return renderLoopsMode(
    analysisGraph,
    visualizationData.loopDefinitions,
    onElementClick,
    sectionStyle
  );
}
...
function handleCutSetsMode(
  ...
  return renderCutSetsMode(
    analysisGraph,
    visualizationData.cutSetDefinitions,
    onElementClick,
    sectionStyle
  );
}
