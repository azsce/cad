...
function extractLoopIndex(referenceId: string): number | null {
  ...
  return loopMatch?.[1] ? parseInt(loopMatch[1], 10) : null;
}
...
function extractCutsetIndex(referenceId: string): number | null {
  ...
  return cutsetMatch?.[1] ? parseInt(cutsetMatch[1], 10) : null;
}
...
function highlightElement(
  ...
  const element = type === 'loop' 
    ? visualizationData.loopDefinitions?.[index]
    : visualizationData.cutSetDefinitions?.[index];
...
function useMarkdownComponents() {
  ...
  const handleReferenceClick = useCallback(
    (referenceId: string) => {
      ...
      if (loopIndex !== null) {
        const highlighted = highlightElement(loopIndex, visualizationData, setHighlightedElements, 'loop');
        if (highlighted) {
          return;
        }
      }

      // Try to highlight cut-set
      ...
      <Box sx={containerStyle}>
        ...
        <Box sx={emptyStateStyle}>
          ...
          <Typography variant="body2">
            Click "Run Analysis" to see the step-by-step solution
          </Typography>
        </Box>
      </Box>
    );
