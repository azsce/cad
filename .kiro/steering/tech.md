...
- Usage requires a `caller` identifier:
  ```typescript
  logger.info({ caller: 'ComponentName' }, 'Message', { context: 'data' });
  logger.error({ caller: 'ComponentName' }, 'Error occurred', error);
  logger.warn({ caller: 'ComponentName' }, 'Warning message');
  logger.debug({ caller: 'ComponentName' }, 'Debug info', debugData);
  ```
